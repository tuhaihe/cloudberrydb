...
def run_exkeys(hosts, capture=False):
    ...
    #
    # The overview: we spawn two tee(1) processes and connect their standard
    # output to the stdout/err of the current Python process. We then connect
    # the stdout/stderr streams of subprocess.call() to the stdin of those tee
    # processes. tee(1) will duplicate all output to temporary files, which we
    ...
    with tempfile.NamedTemporaryFile() as temp_out, tempfile.NamedTemporaryFile() as temp_err:
        tee_out = subprocess.Popen(['tee', temp_out.name], stdin=subprocess.PIPE, stdout=sys.stdout)
        tee_err = subprocess.Popen(['tee', temp_err.name], stdin=subprocess.PIPE, stdout=sys.stderr)
