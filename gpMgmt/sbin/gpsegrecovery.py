...
class DifferentialRecovery(Command):
    ...
    def run(self):
        ...
        try:
            ...
            # Backup is completed, now run pg_backup_stop which will also remove backup_label file from
            # primary data_dir
    ...
    def sync_pg_data(self):
        ...
        """
            ...
            2. backup_label is present in excludeFiles list in src/backend/backup/basebackup.c as in case of 
               pg_basebackup, pg_backup_start() is queried in non-exclusive mode. pg_backup_start() in non-exclusive 
               mode does not create backup_label file so the backup_label (if present) belongs to other user who ran 
               pg_backup_start() in exclusive mode. backup_label is required by mirror to retrieve the wal location it 
            ...
            3. backup_label has not been added in rsync_exclude_list that means it is sent to mirror during pg_data sync. 
               Reasons behind not excluding backup_label from pg_data sync:
                1. pg_backup_start() is executed in exclusive mode only by differential recovery which creates
                ...
                2. So It's safe to copy backup_label file to mirror knowing that it is created by differential recovery 
                   as no other greenplum utility executes pg_backup_start() in exclusive mode at present.
                   
            4. In future we will have to add backup_label in rsync_exclude_list if pg_backup_start needs be started in
        ...
        rsync_exclude_list = {
            ...
            "postmaster.opts",
            "pg_dynshmem",
            "tablespace_map", # this files getting generated on call of pg_backup_start
            "pg_notify/*",
            "pg_replslot/*",
            "pg_serial/*",
            "pg_stat_tmp/*",
    ...
    def pg_backup_start(self):
        sql = "SELECT pg_backup_start('differential_backup', false);"
        try:
            RemoteQueryCommand("Start backup", sql, self.recovery_info.source_hostname,
                               self.recovery_info.source_port).run()
        except Exception as e:
            raise Exception("Failed to query pg_backup_start() for segment with host {} and port {} : {}".format(
                self.recovery_info.source_hostname,
                self.recovery_info.source_port, str(e)))
        self.logger.debug("Successfully ran pg_backup_start for segment on host {}, port {}".
                          format(self.recovery_info.source_hostname, self.recovery_info.source_port))

    def pg_backup_stop(self):
        sql = "SELECT pg_backup_stop();"
        try:
            RemoteQueryCommand("Stop backup", sql, self.recovery_info.source_hostname,
                               self.recovery_info.source_port).run()
        except Exception as e:
            raise Exception("Failed to query pg_backup_stop() for segment with host {} and port {} : {}".
                            format(self.recovery_info.source_hostname, self.recovery_info.source_port, str(e)))

        self.logger.debug("Successfully ran pg_backup_stop for segment on host {}, port {}".
                          format(self.recovery_info.source_hostname, self.recovery_info.source_port))
